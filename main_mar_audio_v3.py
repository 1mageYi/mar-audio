...
def get_args_parser():
    ...
    parser.add_argument('--overfit_one_batch', action='store_true',
                        help='If true, runs the model on a single batch to check for overfitting.')
...
def main(args):
    ...
    if args.overfit_one_batch:
        print("\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
        print("!!!           RUNNING IN OVERFIT-ONE-BATCH-MODE           !!!")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n")
        # 1. 从原始数据集中取出一个批次的数据
        single_batch_loader = torch.utils.data.DataLoader(dataset_train, batch_size=args.batch_size)
        single_batch = next(iter(single_batch_loader))
        
        # 2. 创建一个只包含这个批次数据、并会无限重复的新数据集
        class OverfitDataset(Dataset):
            ...
            def __len__(self):
                # 我们可以让它看起来像有10000个样本，实际上都是重复的
                return 10000

            def __getitem__(self, index):
                # 无论索引是什么，都返回该批次中的一个样本
                idx_in_batch = index % self.num_samples
                return self.batch[0][idx_in_batch], self.batch[1][idx_in_batch]
        
        # 3. 用这个特殊的数据集替换原始数据集
        dataset_train = OverfitDataset(single_batch)
